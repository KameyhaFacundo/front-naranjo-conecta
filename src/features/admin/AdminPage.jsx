...
import ResumenPanel from './ResumenPanel.jsx'
import UsuariosPanel from './UsuariosPanel.jsx'

const ESTADOS = ['pendiente', 'en_revision', 'en_proceso', 'resuelto']

const VISTAS = [
  { clave: 'resumen', label: 'Resumen', icono: 'objetivo' },
  { clave: 'publicaciones', label: 'Publicaciones', icono: 'lista' },
  { clave: 'usuarios', label: 'Usuarios', icono: 'escudo' },
]
  ...
  return (
    <section className="panel-admin">
      <header className="seccion-header">
        ...
        <p>Moderá las publicaciones y controlá las cuentas desde un solo lugar.</p>
      </header>

      <div className="pestañas pestañas-panel">
        {VISTAS.map((v) => (
          <button
            key={v.clave}
            type="button"
            className={v.clave === vista ? 'activa' : ''}
            onClick={() => setVista(v.clave)}
          >
            <Icon name={v.icono} size={16} /> {v.label}
          </button>
        ))}
      </div>

      {vista === 'resumen' && <ResumenPanel />}
